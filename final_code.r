...
library(readr)
historic_data <- read_csv("https://raw.githubusercontent.com/aishwaryjoshi/fin550_big_data_for_finance/refs/heads/main/historic_property_data.csv")
predict_data <- read_csv("https://raw.githubusercontent.com/aishwaryjoshi/fin550_big_data_for_finance/refs/heads/main/predict_property_data.csv")
...
# Assign rf_sale_price to final_sale_price before rounding
predict_data$final_sale_price <- predict_data$rf_sale_price

# Round the 'final_sale_price' to the nearest integer and ensure two decimal places
predict_data$final_sale_price <- sprintf("%.2f", round(predict_data$final_sale_price, 2))
...
write.csv(output_data, "predict_data_with_predictions.csv", row.names = FALSE)

# End time logging
end_time <- Sys.time()
cat("Script ended at:", format(end_time, "%Y-%m-%d %H:%M:%S"), "\n")

# Calculate and print the duration
duration <- end_time - start_time
cat("Total time taken to run the script:", duration, "\n")
